...
class IssueController {
  ...
  // Get single issue by ID (with view increment)
  async getIssueById(req, res, next) {
    try {
      const { id } = req.params;
      const issue = await issueService.getIssueById(id);
  ...
  async toggleUpvote(req, res, next) {
    try {
      ...
      const result = await issueService.toggleUpvote(id, userId);
  ...
  // Report spam with improved deduplication
  async reportSpam(req, res, next) {
    try {
      const { id } = req.params;
      ...
      const result = await issueService.reportSpam(id, userId, reason, description);
