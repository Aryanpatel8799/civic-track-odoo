...
const { validate, validateFormData, issueCreateSchema, issueUpdateSchema, spamReportSchema } = require('../middlewares/validate');
const { upload } = require('../utils/fileUtils');
const { spamReportLimiter, issueCreateLimiter } = require('../middlewares/rateLimiter');
...
router.post(
  ...
  validateFormData(issueCreateSchema),
...
// @access  Private
router.post('/:id/upvote', auth, requireUser, issueController.toggleUpvote);
